            ...
            <div class="card-body">
                ...
                <div class="row">
                    ...
                    <div class="col-lg-3 col-md-3 col-sm-3">
                        <label for="nick">*Nome guerra</label>
                        <input class="form-control" placeholder="Nome guerra" value="{{ old('nick', $user->nick ?? '') }}" name="nick" required="">
                    </div>
                ...
                <div class="row">
                    <div class="col-lg-6">
                        <label for="orgmil">*OM / Grupo</label>
                        <select name="orgmil_id" class="form-control">
                            <option value="">Selecione...</option>
                            @foreach ($orgmils as $orgmil)
                                @if(isset($user))
                                    <option value="{{$orgmil->id}}" {{($orgmil->id == $user->orgmil_id ? 'selected=""' : '')}}>
                                        {{$orgmil->name}}
                                    </option>
                                @else
                                    <option value="{{$orgmil->id}}">
                                        {{$orgmil->name}}
                                    </option>
                                @endif
                            @endforeach
                    ...
                    <div class="col-lg-3">
                        <label for="system">*Sistemas </label>
                        <select name="system" data-name="Tipo de uso" class="form-control" required>
                            <option value="">Selecione...</option>
                            @if(isset($user))
                                <option value="1" {{($user->system == "1" ? 'selected=""' : '')}}>
                                    SISCRED / CERBERUS
                                </option>
                                <option value="0" {{($user->system == "0" ? 'selected=""' : '')}}>
                                    CERBERUS
                                </option>
                            @else
                                <option value="1">SISCRED / CERBERUS</option>
                                <option value="0">CERBERUS</option>
                            @endif
                        </select>
                    </div>
                </div>
